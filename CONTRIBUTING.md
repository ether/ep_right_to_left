# Contributing to ep_right_to_left

Thanks for helping improve `ep_right_to_left` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_right_to_left` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_right_to_left from your local clone
pnpm run plugins i --path ../ep_right_to_left

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_right_to_left/` are picked up after a server restart.
...
* Etherpad version, ep_right_to_left version, browser + OS
* Steps to reproduce
